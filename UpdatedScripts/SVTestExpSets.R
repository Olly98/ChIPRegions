...
for(i in 1:length(Model)){
  ...
  if(length(HMM)==19){
    ...
    Furf=findOverlaps(Slam,Refs[[SetsMeta$UnfoldedCells[i]]])
    Frame=data.frame(Slam$State[Furf@from],Refs[[SetsMeta$UnfoldedCells[i]]]$V4[Furf@to])
    colnames(Frame)=c("State","SV")
    if(length(unique(Frame$SV))<2){
      SVTest=c(SVTest,setNames(list(unique(Frame$SV)),paste(names(Model)[i] ) ))
      SigTest=c(SigTest,setNames(list(unique(Frame$SV)),paste(names(Model)[i] ) ))
    }else{
      ...
      print(SetsMeta$UnfoldedCells[i])
      ...
      for(k in 1:10000){
        ...
        Mlap=Refs[[SetsMeta$UnfoldedCells[i]]]
      ...
      SVTest=c(SVTest,setNames(list(test),paste(names(Model)[i] ) ))
      SigTest=c(SigTest,setNames(list(tempt),paste(names(Model)[i] ) ))}
